%-------------------------------------------------------------------------%
%-- Script for plotting point spread function of a cross
%-- array (CA) and an L array placed at the corner (LA) of the planar array for the
%-- product beamforming
...
%% Uncomment the  following for the PSF of LA
delay_V=squeeze(delay(1,:,:,:));  % delay for the L array
delay_H=squeeze(delay(:,1,:,:));
Sensor_data_V=squeeze(Sensor_data(:,1,:));   % Sensor data for the L array Data
Sensor_data_H=squeeze(Sensor_data(:,:,1));
%% Uncomment the  following for the PSF of CA
% delay_V=squeeze(delay(M/2,:,:,:));  % delay for Cross array
% delay_H=squeeze(delay(:,N/2,:,:));
% Sensor_data_V=squeeze(Sensor_data(:,M/2,:));   %  Sensor data for the cross array Data
...
tic
beamformed_data=VHLABF_Cross(Sensor_data_V,Sensor_data_H,timeVector,delay_V,delay_H,range_index);
toc
